...
public partial class Admin_Default2 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Page.IsPostBack == false)
        {
            ...
            con.Open();
            SqlDataAdapter sd = new SqlDataAdapter("select m.ID,m.NAME,m.BANKNAME as BANK,m.ACNO,m.IFSC,(select isnull(sum(amount),0) from wallet where (stat='D' or stat='B') and id=m.id) as AMOUNT,cast((select isnull(sum(amount),0) from wallet where (stat='D' or stat='B') and id=m.id)*.15 as decimal(10,2)) as 'CHG',cast((select isnull(sum(amount),0) from wallet where (stat='D' or stat='B') and id=m.id)*.9 as decimal(10,2)) as NET,(select isnull(sum(amount),0) from wallet where stat='P' and id=m.id) as PAID,((select isnull(sum(amount),0) from wallet where (stat='D' or stat='B') and id=m.id)*.9)-(select isnull(sum(amount),0) from wallet where stat='P' and id=m.id) as 'TO PAY' from member_creation m where (((select isnull(sum(amount),0) from wallet where (stat='D' or stat='B') and id=m.id)*.9)-(select isnull(sum(amount),0) from wallet where stat='P' and id=m.id))>0 order by m.id", con);
